...
def generate_index(request: GenerateIndexRequest):
    try:
        ...
        index_information = {
            "index_id": STORE.generate_index_uuid(),
...
def get_index_status(index_id: str):
    try:
        return {
            ATTR_STATUS: STORE.get_index_information(index_id=index_id)[
                ATTR_STATUS
            ]
        }
    except KeyError:
...
def get_documents(request: RetrieveRequest):
    try:
        ...
        if request.index_id:
            ...
            index_information = STORE.get_index_information(
            ...
            retriever_kwargs[
                "index_root"
            ] = STORE.get_index_directory_path(request.index_id)
