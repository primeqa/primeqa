...
from primeqa.ir.dense.colbert_top.colbert.indexing.codecs.residual_embeddings_strided import (
    ResidualEmbeddingsStrided,
)
from primeqa.ir.dense.colbert_top.colbert.search.strided_tensor import StridedTensor
from primeqa.ir.dense.colbert_top.colbert.search.candidate_generation import (
    CandidateGeneration,
)
from .index_loader import IndexLoader
from primeqa.ir.dense.colbert_top.colbert.modeling.colbert import (
    colbert_score,
            ...
            for i in range(0, ceil(len(pids) / batch_size)):
                ...
                if approx_scores_.shape[0] == 0:
                    approx_scores.append(
                        torch.zeros((len(pids_),), dtype=approx_scores_.dtype)
                    )
                    continue
