...
def get_lang_map(tsv_file):
    ...
    the language in the URL. @param tsv_file: The path to the TSV (Tab-Separated Values) file containing language
    mappings. @return: A generator that yields tuples of id and language. Each tuple represents a language mapping
    found in the TSV file.
    """
...
def close_files(file_map):
    ...
    This method iterates through the keys of the file_map dictionary and closes each file object associated with each
    key.
    """
    for l in file_map.keys():
        file_map[l].close()
...
def main():
    ...
    lang_map = {}
    seen = {}
    for _id, lang in get_lang_map(args.tsv):
        lang_map[_id] = lang
        seen[lang] = 1
