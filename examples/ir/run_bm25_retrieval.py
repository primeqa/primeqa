...
def handle_args():
    ...
    parser.add_argument('--index_path', required=True,help="Path to a lucene index")
    parser.add_argument('--queries_file', required=True, help='Path to queries file in ColBERT tsv [id\tquery] format')
    parser.add_argument('--top_k', required=False, default=1000, type=int, help='Number of documents/passages to retrieve')
    parser.add_argument('--xorqa_data_file', required=False, default=None, help="Required to extract language id for XORQA evaluation." )
...
def run_search(index_path, queries, top_n=1000, k1=0.9, b=0.4):
    logger.info(f"Loading index {index_path}")
    searcher = PyseriniRetriever(index_path, k1=k1, b=b)
...
def write_colbert_ranking_tsv(output_dir, id_to_hits):
...
def get_language_id(xorqa_data_file):
    id_to_lang = {}
...
def write_xorqa_json(output_dir, id_to_hits,top_n=100, xorqa_data_file=None):
    id_to_lang = None
    ...
    for id in id_to_hits:
        ...
        for hit in id_to_hits[id][0:top_n]:
...
def write_search_results(id_to_hits, output_dir, max_hits_output, xorqa_data_file=None):
