...
from primeqa.mrc.models.heads.extractive import (
    EXTRACTIVE_HEAD,
    EXTRACTIVE_WITH_CONFIDENCE_HEAD,
)
...
class ModelArguments:
    ...
    confidence_model_path: str = field(
        default=None, metadata={"help": "Path to the confidence calibration model"}
    )
...
class DataTrainingArguments:
    ...
    data_file_format: str = field(
        default="json",
        metadata={
            "help": "the format of the local dataset files (json, csv, text, pandas)"
        },
    )
    dataset_config_name: str = field(
        default="primary_task",
        metadata={
...
class TaskArguments:
    ...
    task_heads: object_reference = field(
        default=None,
        metadata={
            "help": "The name of the task head to use.",
    ...
    do_boolean: bool = field(
        default=False,
        metadata={
            "help": "Enable processing of boolean questions.  If activated,"
    ...
    boolean_config: str = field(
        default=None,
        metadata={
            "help": "The configuration name file for the boolean task in json format"
        },
    )
    passage_non_null_threshold: int = field(
        default=2,
        metadata={
            "help": "The passage level non-null threshold (number of annotators to indicate no answer). This should be set to 1 if there is only one annotation"
        },
    )
    span_non_null_threshold: int = field(
        default=2,
        metadata={
            "help": "The span level non-null threshold (number of annotators to indicate no answer). This should be set to 1 if there is only one annotation"
        },
    )
    verbose: bool = field(
        default=False,
    ...
    output_dropout_rate: float = field(
        default=0.25,
        metadata={
            "help": "The dropout probability applied to LM output in "
    ...
    decoding_times_with_dropout: int = field(
        default=5,
        metadata={
            "help": "The number of decoding times to generate confidence "
    ...
    def __post_init__(self):
        if not self.task_heads:
            self.task_heads = (
                EXTRACTIVE_HEAD  # cannot directly set mutable value as default
            )
...
def main():
    ...
    def compute_metrics(p: EvalPredictionWithProcessing):
        return eval_metrics.compute(
            ...
            span_non_null_threshold=task_args.span_non_null_threshold,
    ...
    def preprocess_fn_wrapper(
        process_fn: Callable, add_examples_to_output: bool = False
    ...
    rc_trainer_config = TrainerConfig(
        ...
        rc_trainer_class,
        rc_trainer_kwargs,
        preprocess_fn_wrapper(preprocessor.process_train),
        preprocess_fn_wrapper(preprocessor.process_eval),
        None,
    )
    ...
    gen_trainer_config = TrainerConfig(
        ...
        GenTrainer,
        gen_trainer_kwargs,
        preprocess_fn_wrapper(qgdl.create, add_examples_to_output=True),
        preprocess_fn_wrapper(qgdl.create, add_examples_to_output=True),
        None,
    )
    ...
    al = ActiveLearner(
        training_args.output_dir, (rc_trainer_config, gen_trainer_config)
    )
    ...
    strategy = GenALScorer(
        strategy=GenALScorer.Strategy.ROUNDTRIP,
        max_gen_length=30,
