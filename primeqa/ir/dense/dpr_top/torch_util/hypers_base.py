...
def dist_initialize():
    ...
    if "RANK" not in os.environ:
        ...
        if env_master_addr.startswith('file://'):
            ...
            torch.distributed.init_process_group(backend='nccl')
            # CONSIDER support gloo  torch.distributed.init_process_group(backend='gloo')
...
class HypersBase:
    ...
    def _setup_logging(self):
        ...
        transformers.logging.set_verbosity_error()  # to suppress excess tokenizer warnings about overflowing tokens
    ...
    def jsonl_instances(self, filename: str, *,
                        rand: Optional[random.Random],
                        filter_out: Optional[Callable[[Dict], bool]] = None):
        """
        Provides json objects for one pass over 'filename'. Only instances for our global rank are returned
        :param filename: the file or directory of the jsonl dataset
        :param rand: the random.Random, should be seeded from hypers.seed, None if no shuffling
        ...
        for line_ndx, line in enumerate(dataset):
            ...
            if filter_out is not None and filter_out(jobj):
                continue
            if line_ndx % self.world_size != self.global_rank:
                continue
            yield jobj
