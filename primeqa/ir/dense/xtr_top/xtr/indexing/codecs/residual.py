...
from primeqa.ir.dense.xtr_top.xtr.indexing.codecs.residual_embeddings import ResidualEmbeddings
...
class ResidualCodec:
    ...
    @classmethod
    def load(cls, index_path, config=None):
    ...
    def compress(self, embs):
        ...
        for batch in embs.split(1 << 18): #2**18
            ...
            #difference b/w an embedding and its closest centroid
            residuals_ = (batch - centroids_)

            codes.append(codes_.cpu())
            approximate_residuals = self.binarize(residuals_).cpu()
            residuals.append(approximate_residuals)
    ...
    def compress_into_codes(self, embs, out_device):
        ...
        for batch in embs.split(bsize):
            ...
            #indices -> indices are centroid ids to which each embedding belongs
            if self.use_gpu:
