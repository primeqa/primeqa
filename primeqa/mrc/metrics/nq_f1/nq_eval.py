...
def main(predictions_path, grount_truth_file_pattern, num_workers, use_pretty_print,
         skip_missing_example_ids: bool = False, long_non_null_threshold: int = 2,
         short_non_null_threshold: int = 2):
    nq_gold_dict = load_gt_lookup_as_dict(grount_truth_file_pattern, num_workers=num_workers)
    nq_pred_dict = util.read_prediction_json_from_file(predictions_path)

    long_answer_stats, short_answer_stats = score_answers(
        gold_annotation_dict=nq_gold_dict, pred_dict=nq_pred_dict,
        skip_missing_example_ids=skip_missing_example_ids,
        long_non_null_threshold=long_non_null_threshold,
        short_non_null_threshold=short_non_null_threshold)

    if use_pretty_print:
        pretty_print(long_answer_stats=long_answer_stats, short_answer_stats=short_answer_stats)
...
def _parse_cmd_line_args():
    ...
    parser.add_argument("--gold_path", type=str, required=True,
                        help="filepath pattern to the gzipped json files containing ground truth")
    parser.add_argument("--predictions_path", type=str, required=True,
                        help="The json file containing the model predictions to be scored. "
                             "File format:"
                             "{'predictions': [                                    "
                             "  {                                                  "
                             ...
                             "  }, ... ]                                           "
                             "}")
    parser.add_argument("--short_non_null_threshold", default=2, type=positive_integer_type,
                        help='Min number of non null spans in the annotations before considering'
                             ' an answer as needing a "correct answer"')
    parser.add_argument("--long_non_null_threshold", default=2, type=positive_integer_type,
                        help='Min number of non null spans in the annotations before considering'
                             ' an answer as needing a "correct answer"')
    parser.add_argument("--skip_missing_ids", action='store_true',
                        help='Discard any example ids that are in the gold dict but not in the'
                             ' predictions')
    parser.add_argument("--num_threads", type=int, default=10, help="Number of threads for reading")
    parser.add_argument("--pretty_print", action='store_true')
    parser.add_argument("--verbose_logging", const=logging.DEBUG, default=logging.INFO,
                        action='store_const', dest='loglevel', help="Set log level to debug")
...
if __name__ == '__main__':
    ...
    main(predictions_path=args.predictions_path, grount_truth_file_pattern=args.gold_path,
         num_workers=args.num_threads, skip_missing_example_ids=args.skip_missing_ids,
         short_non_null_threshold=args.short_non_null_threshold,
         long_non_null_threshold=args.long_non_null_threshold, use_pretty_print=args.pretty_print)
