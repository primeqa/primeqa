...
data_df = pd.read_csv(default_rank_dir + "/output_withgold.csv", header=0)
...
reader = ExtractiveReader(model="/dccstor/srosent2/primeqa/experiments/reader_reranker/2epochRoberta/SAPnew/NQftSAP_lr4e-5_1ep-.05-0.20negs/output/", max_num_answers=60, scorer_type=SupportedSpanScorers.SCORE_DIFF_BASED.value, n_best_size=1, max_answer_length=256, min_answer_length=34, stride=64)
reader.load()
...
with open(f'{output_dir}/reader_answersnewftSAPlr4e-5negsnqformat_withgold.json', "w") as outfile: # _{start}-{end}.json
