...
@dataclass
class BertScoreReranker(BaseReranker):
    """_summary_
    ...
    def predict(self, queries: List[str], 
        ...
        for query, docs  in zip(queries,documents):
            ...
            a1_terms = query.split()
            b1_terms = []
            for t in texts_b:
                b1_terms.append(t.split())
                
            sim = len(a1_terms & b1_terms) / len(a1_terms | b1_terms)
            sim += self._bert_scorer.score(texts_a, texts_b)[0].item()
            scores = []
