...
chosen_BEIR_set = "dbpedia-entity"
...
def end_to_end_reranker_training(given_prompt, given_device, given_process_number, queue, synthetic_queries_filename, synthetic_qas_filename, zeroshot_ranking):

	print("Starting end-to-end process!")

	reranker_checkpoint_path, reranker_results_filename, reranker_success_at_five, baseline_success_at_5 = train_reranker(zeroshot_ranking, synthetic_queries_filename, synthetic_qas_filename, chosen_split, chosen_type, chosen_set, given_device, given_process_number, LoTTE_or_BEIR, chosen_BEIR_set, chosen_BEIR_type)

	print("Completed step #3!")

	reranker_performance, baseline_performance = evaluate_reranker(reranker_checkpoint_path, chosen_split, chosen_type, chosen_set, given_device, LoTTE_or_BEIR, chosen_BEIR_set, chosen_BEIR_type)
	...
	triples_for_distillation = generate_triples(reranker_results_filename, chosen_split, chosen_type, chosen_set, given_process_number, LoTTE_or_BEIR, chosen_BEIR_set, chosen_BEIR_type)
...
def combine_rerankers_and_evaluate(triples_and_queries_list):
	...
	for triples_and_queries in triples_and_queries_list:
		...
		distilled_checkpoint = distill_triples_with_retriever(triples_and_queries[0], triples_and_queries[1], distilled_checkpoint, chosen_split, chosen_type, chosen_set, LoTTE_or_BEIR, chosen_BEIR_set, chosen_BEIR_type)

	########################################

	print("Completed step #6!")

	distilled_ranking = generate_ColBERTv2_zeroshot_results(None, distilled_checkpoint, chosen_split, chosen_type, chosen_set, -1, LoTTE_or_BEIR, chosen_BEIR_set, chosen_BEIR_type, re_index=True)

	print("Completed step #7!")

	if LoTTE_or_BEIR == "LoTTE":

		evaluate_dataset(chosen_type, chosen_split, chosen_set, 5, 
			             "../ColBERT_FM/downloads/lotte/" + chosen_split + "/" + chosen_set + "/qas." + chosen_type + ".jsonl", 
			             distilled_ranking)

		evaluate_dataset(chosen_type, chosen_split, chosen_set, 20, 
			             "../ColBERT_FM/downloads/lotte/" + chosen_split + "/" + chosen_set + "/qas." + chosen_type + ".jsonl", 
			             distilled_ranking)

		evaluate_dataset(chosen_type, chosen_split, chosen_set, 100, 
			             "../ColBERT_FM/downloads/lotte/" + chosen_split + "/" + chosen_set + "/qas." + chosen_type + ".jsonl", 
			             distilled_ranking)
...
if __name__ == '__main__':
    ...
    for prompt, chosen_device, process_number in zip(unique_prompts, devices, range(len(unique_prompts))):
        ...
        synthetic_queries_filename, synthetic_qas_filename = generate_synthetic_queries(prompt, model_choice, query_count, chosen_split, chosen_type, chosen_set, 
        ...
        zeroshot_ranking = generate_ColBERTv2_zeroshot_results(synthetic_queries_filename, "DSP_Experiments/msmarco.psg.kldR2.nway64.ib__colbert-400000", 
        													   chosen_split, chosen_type, chosen_set, process_number, LoTTE_or_BEIR, chosen_BEIR_set, chosen_BEIR_type, re_index=True)
    ...
    for prompt, chosen_device, process_number, synthetic_queries_filename, synthetic_qas_filename, zeroshot_ranking in zip(unique_prompts, devices, range(len(unique_prompts)), synthetic_queries_filenames, synthetic_qas_filenames, zeroshot_rankings):
        ...
        process = context.Process(target=end_to_end_reranker_training, args=(prompt,chosen_device,process_number,queue, synthetic_queries_filename, synthetic_qas_filename, zeroshot_ranking, ))
