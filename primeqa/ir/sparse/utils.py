...
def load_queries(queries_tsv_filepath):
    queries = {}
    with open(queries_tsv_filepath, "r", encoding="utf-8") as f:
...
def get_language_id(xorqa_data_file: str) -> Dict:
    id_to_lang = {}
    with open(xorqa_data_file, "r", encoding="utf-8") as f:
        for line in f:
...
def write_xorqa_json(
    output_dir: str,
    id_to_hits: Dict,
    top_n: int = 100,
    xorqa_data_file: str = None,
):
    id_to_lang = None
    if xorqa_data_file:
    ...
    for id in id_to_hits:
        json_data = {
            "id": id,
            "lang": id_to_lang[id] if id_to_lang else "",
            "ctxs": [],
        }
...
def write_search_results(
    ...
    write_xorqa_json(
        output_dir,
        id_to_hits,
        top_n=max_hits_output,
        xorqa_data_file=xorqa_data_file,
    )
