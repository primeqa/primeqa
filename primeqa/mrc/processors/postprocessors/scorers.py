...
class SupportedSpanScorers(Enum):
    ...
    WEIGHTED_SUM_SHORT_LONG_TARGET_TYPE_AND_SCORE_DIFF = 'weighted_sum_short_long_target_type_and_score_diff'
...
def initialize_scorer(scorer_type: Union[str, SupportedSpanScorers],
    ...
    elif scorer_type == SupportedSpanScorers.WEIGHTED_SUM_SHORT_LONG_TARGET_TYPE_AND_SCORE_DIFF:
        logging.debug(
            "\tInitialized scorer %s with weight %s for the short and long answer target type score and weight %s for "
            "the score diff score" % (
                compute_weighted_sum_short_and_long_answer_type_score_diff_between_span_and_cls.__name__,
                target_type_weight, 1 - target_type_weight))

        return partial(
            compute_weighted_sum_short_and_long_answer_type_score_diff_between_span_and_cls,
            target_type_weight=target_type_weight)
    else:
...
def compute_weighted_sum_short_and_long_answer_type_score_diff_between_span_and_cls(
        span_score: float, null_span_score: float,
        target_type_logits: List, target_type_weight: float, *args, **kwargs):
    """
    Compute score as sum of products `target_type_weight` times target type logit (for span answer) and
    ...
    return (1 - target_type_weight) * score_diff + target_type_weight * \
           (target_type_logits[int(TargetType.PASSAGE_ANSWER)] +
            target_type_logits[int(TargetType.SPAN_ANSWER)])
