from tableQG.wikisql_lib.common import detokenize
...
class Query:
    ...
    def __repr__(self):
        ...
        if self.conditions:
            rep += ' WHERE ' + ' AND '.join(['{} {} {}'.format(
                'col{}'.format(i), self.cond_ops[o], v) for i, o, v in self.conditions])
        return rep
    ...
    def from_sequence(cls, sequence, table, lowercase=True):
        ...
        if 'symend' in sequence['words']:
            ...
            for k, v in sequence.items():
                sequence[k] = v[:end]
        terms = [{'gloss': g, 'word': w, 'after': a} for g, w, a in zip(
            sequence['gloss'], sequence['words'], sequence['after'])]
        ...
        def flatten(tokens):
            ...
            return ret
        where_index = [i for i, t in enumerate(
            terms) if t['word'] == 'symwhere']
        ...
        try:
            ...
            raise Exception(
                'Cannot find aggregation column {}'.format(flat['words']))
        ...
        while where_terms:
            ...
            if t['word'] != 'symcol':
                raise Exception(
                    'Missing conditional column {}'.format(flat['words']))
            try:
                op_index = flat['words'].index('symop')
                col_tokens = flatten(where_terms[:op_index])
            except Exception as e:
                raise Exception(
                    'Missing conditional operator {}'.format(flat['words']))
            ...
            try:
                ...
                raise Exception(
                    'Cannot find conditional column {}'.format(col_tokens['words']))
            try:
                val_index = flat['words'].index('symcond')
            except Exception as e:
                raise Exception(
                    'Cannot find conditional value {}'.format(flat['words']))

            where_terms = where_terms[val_index+1:]
            flat = flatten(where_terms)
            val_end_index = flat['words'].index(
                'symand') if 'symand' in flat['words'] else len(where_terms)
