...
class IndexingArguments:
    ...
    overwrite: bool = field(
        default=False, metadata={"help": "Overwrite existing directory"}
    )

    collection: str = field(
        default=None,
        metadata={"help": "Path to a corpus tsv or json file or directory"},
    )

    fieldnames: list = field(
        default=None,
        metadata={
            "help": "fields names to use to identify document_id, title, text if corpus tsv has no headings"
        },
    )

    additional_indexing_args: str = field(
        default="--storePositions --storeDocvectors --storeRaw",
        metadata={"help": "pyserini index options"},
    )
...
class SearchArguments:
    ...
    k1: float = field(
        default=0.8,
        metadata={"help": "bm25 parameter to tune impact of term frequency"},
    )

    b: float = field(
        default=0.4,
        metadata={
            "help": "bm25 constant to fine tune the effect of document length"
        },
    )
