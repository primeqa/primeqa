...
def write_examples_as_triples(qas, max_num_positives, max_num_negatives, add_title_text, num_rounds, randomize, output_dir):
    
    df = convert_json_to_df(qas, max_num_positives=max_num_positives, max_num_negatives=max_num_negatives, add_title_text=add_title_text)
    ...
    filename = os.path.join(output_dir, f"xorqa_triples_3poss_100neg_{num_rounds}ep_rand{randomize}.tsv")
    f = open(filename,'w',encoding='utf-8') 
    filename = os.path.join(output_dir, f"xorqa_triples_3poss_100neg_en_{num_rounds}ep_rand{randomize}.tsv")
    ...
    if randomize:
        ...
        logger.info(f'Writing num rounds {num_rounds}, {len(indexes)} randomized triples to {f.name}')
        df[cols].iloc[indexes].to_csv(f,
                             sep='\t',
                             index=None,
                             header=None)
        logger.info(f'Writing num rounds {num_rounds}, {len(indexes)} randomized triples to {f_en.name}')
        df[cols_en].iloc[indexes].to_csv(f_en,
                             sep='\t',
                             index=None,
                             header=None)
    else:
...
def run_bm25_retrieval(searcher, answer_checker, qas, question_translations, args):
    ...
    for qnum, qa in enumerate(tqdm(qas)):
        ...
        for hit in retrieved:
            title = hit['title']
            text = hit['text']
            string_to_search = text if args.do_not_run_match_in_title else title + ' ' + text
            if answer_checker.has_answer(qa['answers'], string_to_search):
                if len(positives) < args.max_num_ir_based_positives and not max([ ctxt in text for ctxt in positive_ctxs_texts ]):
        ...
        logger.debug(f"{en_q} adding { len(qa['ir_negative_ctxs']) } IR based negatives { len(qa['ir_positive_ctxs']) } IR based positives")
...
def main():
    ...
    write_examples_as_triples(qas, args.max_num_positives, args.max_num_negatives, args.add_title_text, args.num_rounds, args.randomize, args.output_dir)
