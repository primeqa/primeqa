...
class IndexSaver():
    ...
    def save_codec(self, codec):
        codec.save(index_path=self.config.index_name)

    def load_codec(self):
        return ResidualCodec.load(index_path=self.config.index_name, config=self.config)
    ...
    def _write_chunk_to_disk(self, chunk_idx, offset, compressed_embs, doclens):
        path_prefix = os.path.join(self.config.index_name, str(chunk_idx))
        compressed_embs.save(path_prefix)

        doclens_path = os.path.join(self.config.index_name, f'doclens.{chunk_idx}.json')
