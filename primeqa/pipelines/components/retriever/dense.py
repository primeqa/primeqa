...
@dataclass
class DPRRetriever(RetrieverComponent):
    """_summary_
    ...
    def __post_init__(self):
        self._config = DPRSearchArguments(
            index_location=self.index_root,
    ...
    def retrieve(self, input_texts: List[str], *args, **kwargs):
        ...
        retrieved_doc_ids, passages = self._searcher.search(
            input_texts, max_num_documents, mode="query_list"
        )
        # retrieved_doc_ids: list (per query) of lists (per rank) of docids
        # passages: list (per query) of dicts with keys {'titles', 'texts', 'scores'} of lists (per rank)

        # returning: list (per query) of lists (per rank) of tuples (((nt) docid, (float)score)
        return [
